...
export default async function AdminPayoutsPage() {
  ...
  // 1. Fetch all creators who have bank accounts configured
  ...
  // 2. Fetch all successful transactions to calculate lifetime earnings
  const { data: transactions } = await supabase
    .from('transactions')
    .select('creator_id, creator_share')
    .eq('status', 'success');

  // 3. Fetch all processing/paid payouts to calculate lifetime payouts
  const { data: pastPayouts } = await supabase
    .from('payouts')
    .select('id, creator_id, net_amount, status, created_at, profiles!payouts_creator_id_fkey ( full_name, display_name, email )')
    .in('status', ['processing', 'paid']);

  // Calculate dynamic pending payouts
  ...
  if (creatorProfiles) {
    for (const cProfile of creatorProfiles) {
      ...
      const lifetimeEarnings = (transactions || [])
        ...
        .reduce((sum, t) => sum + (t.creator_share || 0), 0);
        
      const lifetimePayouts = (pastPayouts || [])
        .filter(p => p.creator_id === creatorId)
        .reduce((sum, p) => sum + (p.net_amount || 0), 0);
        
      const unpaidBalance = lifetimeEarnings - lifetimePayouts;
  ...
  // Also fetch recent historical payouts for the ledger (including failed)
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px' }}>
        ...
        <BulkApproveButton creators={pendingPayouts} />
      </div>
      ...
      <div style={{ background: 'var(--v2-surface)', border: '1px solid var(--v2-outline)', borderRadius: '16px', overflow: 'hidden', marginBottom: '40px' }}>
        <div style={{ padding: '24px', borderBottom: '1px solid var(--v2-outline)', background: 'var(--v2-surface-lowest)' }}>
          ...
          <p style={{ fontSize: '14px', color: 'var(--v2-text-variant)' }}>Creators with an unpaid balance ≥ ₦1,000</p>
        </div>
      ...
      <div style={{ background: 'var(--v2-surface)', border: '1px solid var(--v2-outline)', borderRadius: '16px', overflow: 'hidden' }}>
         <div style={{ overflowX: 'auto' }}>
            <div style={{ minWidth: '800px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 2fr 1fr 1fr', padding: '16px 24px', background: 'var(--v2-surface-low)', borderBottom: '1px solid var(--v2-outline)' }}>
                <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--v2-text-variant)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Creator</span>
              ...
              {historicalPayouts.length === 0 ? (
                ...
                return (
                  <div key={payout.id} style={{ display: 'grid', gridTemplateColumns: '2fr 1.5fr 2fr 1fr 1fr', padding: '16px 24px', borderBottom: '1px solid var(--v2-outline)', alignItems: 'center' }}>
                    <div>
